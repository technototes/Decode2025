...
public class SimpleCoaxSwerveDriveSubsystem implements Loggable, Subsystem {
    ...
    public void updateValues(double x, double y, double r) {
        if (hasHardware) {
            ...
            unfilteredWheelPowers[0] = Math.sqrt(B * B + C * C); //Math.sqrt((F * F) + (G * G)) * (r != 0 ? (-rotationInput / r ) : 1);
            unfilteredWheelPowers[1] = Math.sqrt((B * B) + (D * D));
            unfilteredWheelPowers[2] = Math.sqrt(A * A + D * D); //Math.sqrt((E * E) + (H * H)) * (r !=0 ? (-rotationInput / r ) : 1);
