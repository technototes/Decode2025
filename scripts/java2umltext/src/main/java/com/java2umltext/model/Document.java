...
public abstract class Document implements UML {
    ...
    public void addRelationship(Relationship r) {
        if (
            relationshipList
                .stream()
                .noneMatch(i -> r.source().equals(i.source()) && r.target().equals(i.target()))
        ) {
    ...
    public void removeForeignRelations() {
        relationshipList.removeIf(
            r ->
                classList.stream().noneMatch(c -> r.source().equals(c.name())) ||
                classList.stream().noneMatch(c -> r.target().equals(c.name()))
        );
    }
