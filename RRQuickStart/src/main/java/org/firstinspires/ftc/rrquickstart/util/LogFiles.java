package org.firstinspires.ftc.rrquickstart.util;
...
import org.firstinspires.ftc.rrquickstart.drive.SampleMecanumDrive;
import org.firstinspires.ftc.rrquickstart.drive.SampleTankDrive;
import org.firstinspires.ftc.rrquickstart.drive.StandardTrackingWheelLocalizer;
...
public final class LogFiles {
    ...
    public static class LogFile {
        ...
        public RevHubOrientationOnRobot.LogoFacingDirection LOGO_FACING_DIR =
            DriveConstants.LOGO_FACING_DIR;
        public RevHubOrientationOnRobot.UsbFacingDirection USB_FACING_DIR =
    ...
    private static final OpModeManagerNotifier.Notifications notifHandler =
        new OpModeManagerNotifier.Notifications() {
                ...
                while (i < fs.length && totalSizeBytes >= 32 * 1000 * 1000) {
                    ...
                    if (!fs[i].delete()) {
                        RobotLog.setGlobalErrorMsg(
                            "Unable to delete file " + fs[i].getAbsolutePath()
                        );
                    }
                    ++i;
                }
            ...
            public void onOpModePostStop(OpMode opMode) {
                ...
                if (!(opMode instanceof OpModeManagerImpl.DefaultOpMode)) {
                    ...
                    String filename =
                        dateFormat.format(new Date(log.msInit)) +
                        "__" +
                        opMode.getClass().getSimpleName() +
                        ".json";
        ...
        manager.register("/logs/download", session -> {
            ...
            if (pairs.length != 1) {
                return NanoHTTPD.newFixedLengthResponse(
                    ...
                    "expected one query parameter, got " + pairs.length
                );
            }
            ...
            if (!parts[0].equals("file")) {
                return NanoHTTPD.newFixedLengthResponse(
                    NanoHTTPD.Response.Status.BAD_REQUEST,
                    NanoHTTPD.MIME_PLAINTEXT,
                    "expected file query parameter, got " + parts[0]
                );
            }
